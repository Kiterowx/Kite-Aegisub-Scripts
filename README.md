# Kite Aegisub Scripts

A collection of Aegisub automation scripts for timing, quality control, typesetting, motion tracking, vector animation, clip work, gradients, image conversion, snapshots, transform effects, and color management.

## Scripts

| Script | Version | Menu root | Interface languages |
| --- | --- | --- | --- |
| AddTexture | 2.0.13 | `AddTexture` | English |
| Alecto KFX | 3.2.3 | `Alecto KFX` | Spanish |
| AutoBlur | 2.0.9 | `AutoBlur` | English |
| AutoMask | 2.4.7 | `AutoMask` | English |
| Chrono Suite | 1.3.2 | `Chrono Suite` | English, Spanish, Portuguese |
| Cliptomaniac | 0.3.8 | `Cliptomaniac` | English, Spanish |
| Field Group Manager | 1.1.3 | `Field Group Manager` | English |
| Gradient Row | 1.8.3 | `Gradient Row` | English |
| MakeupnPosing | 1.0.0 | `MakeupnPosing` | English, Spanish |
| Moka Motion | 3.6.1 | `Moka Motion` | English |
| Obake | 0.3.5 | `Obake` | English, Spanish |
| PNG2ASS | 1.5.0 | `PNG2ASS` | English |
| Pointiac | 1.1.2 | `Pointiac` | English |
| Rhea Signs | 1.8.5 | `Rhea Signs` | English, Spanish, Portuguese |
| Selesub | 2.1.3 | `Selesub` | English |
| Snapshoter | 1.6.3 | `Snapshoter` | English |
| Wave2json | 1.3.2 | `Wave2json` | No GUI |
| Zagreo Glyphs | 1.1.2 | `Zagreo Glyphs` | English |
| Zheus Colormanager | 4.5.3 | `Zheus Colormanager` | Spanish |

## DependencyControl

DependencyControl update information is provided in `DependencyControl.json`. The feed includes the public macros listed above and the shared `kite.EventOps`, `kite.LineOps`, `kite.Media`, `kite.PyBridge`, `kite.ShapeOptimizer`, `kite.Timing`, and `kite.UI` modules. Stable preferences are stored by namespace in `?user/config/kite.settings.json`.

Feed URL:

```text
https://raw.githubusercontent.com/Kiterowx/Kite-Aegisub-Scripts/main/DependencyControl.json
```

## Public URLs

- Catalogue site: <https://kiterowx.github.io/Guide-Aegisub-Scripts/>
- Chrono Generators: <https://github.com/Kiterowx/Chrono-Generators-Scripts>
- Timing guide: <https://kiterowx.github.io/Arquitectura-del-Timing/>

## Documentation

- [AddTexture](docs/AddTexture.md)
- [Alecto KFX](docs/AlectoKFX.md)
- [AutoBlur](docs/AutoBlur.md)
- [AutoMask](docs/AutoMask.md)
- [Chrono Suite](docs/ChronoSuite.md)
- [Cliptomaniac](docs/Cliptomaniac.md)
- [Field Group Manager](docs/FieldGroupManager.md)
- [Gradient Row](docs/GradientRow.md)
- [MakeupnPosing](docs/MakeupnPosing.md)
- [Moka Motion](docs/MokaMotion.md)
- [Obake](docs/Obake.md)
- [PNG2ASS](docs/PNG2ASS.md)
- [Pointiac](docs/Pointiac.md)
- [Rhea Signs](docs/RheaSigns.md)
- [Selesub](docs/Selesub.md)
- [Snapshoter](docs/Snapshoter.md)
- [Wave2json](docs/Wave2json.md)
- [Zagreo Glyphs](docs/ZagreoGlyphs.md)
- [Zheus Colormanager](docs/Zheus.md)
- [Dependencies](docs/Dependencies.md)

## License

MIT. See [LICENSE](LICENSE).
